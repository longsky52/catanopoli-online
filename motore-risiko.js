// ==========================================
// MOTORE RISIKO CATANESE (TOTALITY GAMES) - V6
// Timer 60s, Battaglie in Diretta (Broadcast), UI Avanzata
// ==========================================
...
window.avviaPartitaRisiko = function(isMulti, dati) {
    ...
    haConquistatoTerritorio = false; rSpostamentoEseguito = false; creaMazzo();
    
    let mapDiv = document.getElementById("risiko-map");
    mapDiv.style.width = "100%"; mapDiv.style.height = "100%"; mapDiv.style.transform = "none"; 
    ...
    containerModals.innerHTML = `
        <div id="risiko-battle-modal">
            <h2 id="r-battle-title" style="color:#ffdf00; font-size: 1.8rem; margin-bottom: 5px; text-transform: uppercase;">BATTAGLIA!</h2>
            <h4 id="r-battle-desc" style="color:#00bfff; font-size: 1rem; margin-top: 0; margin-bottom: 15px;">Quartiere ➔ Quartiere</h4>
            ...
            <div style="display:flex; flex-direction:column; align-items:center; width:100%; gap: 5px;">
                <div style="background: rgba(189,42,42,0.2); border: 2px solid #bd2a2a; border-radius: 8px; padding: 10px; width: 90%;">
                    <h3 id="r-battle-att-name" style="color:#ff4444; margin:0 0 5px 0;">ATTACCO</h3>
                ...
                <div style="background: rgba(44,74,138,0.2); border: 2px solid #2c4a8a; border-radius: 8px; padding: 10px; width: 90%;">
                    <h3 id="r-battle-def-name" style="color:#44ff44; margin:0 0 5px 0;">DIFESA</h3>
    ...
    // 🔥 INIEZIONE TIMER NELL'HUD SUPERIORE 🔥
    let divTitolo = document.getElementById("risiko-fase-titolo").parentNode;
    if(!document.getElementById("risiko-timer-box")) {
        divTitolo.innerHTML = `
            <div style="display:flex; justify-content:center; align-items:center; gap: 15px; width: 100%;">
                <span id="risiko-fase-titolo" style="color: #c99c51; font-weight: bold; font-size: 1.1rem; text-transform: uppercase; letter-spacing: 1px;">INIZIALIZZAZIONE</span>
                <span id="risiko-timer-box">60</span>
            </div>
            <span id="risiko-info-turno" style="color: white; font-size: 0.85rem; margin-top:5px;">Attesa giocatori...</span>
        `;
    }
    ...
    Object.keys(R_NODES).forEach(id1 => {
        ...
        n1.links.forEach(id2 => {
            ...
            if(!disegnati.includes(coppia)) {
                disegnati.push(coppia); let n2 = R_NODES[id2];
                let line = document.createElementNS("http://www.w3.org/2000/svg", "line");
                line.setAttribute("x1", n1.x + "%"); line.setAttribute("y1", n1.y + "%");
                line.setAttribute("x2", n2.x + "%"); line.setAttribute("y2", n2.y + "%");
                line.setAttribute("stroke", "rgba(255,255,255,0.4)"); line.setAttribute("stroke-width", "6"); line.setAttribute("stroke-dasharray", "8,8");
    ...
    Object.keys(R_NODES).forEach(id => {
        let n = R_NODES[id]; let div = document.createElement("div");
    ...
    mioObiettivo = sc.text; mioObiettivoId = sc.id;
    ...
    if(isMulti) {
        ...
        if(socket) {
            ...
            socket.on("riceviTabooAzione", (data) => {
                if(data.isRisiko) {
                    ...
                    // 🔥 RICEZIONE BATTAGLIA IN DIRETTA (SPETTATORI E DIFENSORE) 🔥
                    if(data.azione === "inizio_battaglia") {
                        eseguiAnimazioneBattaglia(data.attId, data.defId, data.resAtt, data.resDef, false);
                        return; // Ferma l'aggiornamento mappa finché l'animazione non finisce!
                    }

                    Object.keys(data.nodes).forEach(k => {
                        R_NODES[k].owner = data.nodes[k].owner; R_NODES[k].troops = data.nodes[k].troops;
                    });
        ...
        for(let i=0; i<dati.bots; i++) { rPlayers.push({ id: "b"+i, name: nomiBot[i], color: rColors[i+1], isBot: true, cards: [] }); }
        if(typeof window.alert === 'function') setTimeout(() => window.alert("IL TUO OBIETTIVO SEGRETO:<br><br>" + mioObiettivo), 500);
        distribuisciTerritoriIniziali(); iniziaTurnoRisiko(0);
    }
};
...
// 🔥 SISTEMA DEL TIMER (60 SECONDI A FASE) 🔥
function avviaTimerRisiko() {
    ...
    timerBox.innerText = secondiRimasti;
    timerBox.style.background = "#bd2a2a"; // Rosso acceso
    
    timerIntervallo = setInterval(() => {
        secondiRimasti--;
        if(secondiRimasti <= 10) timerBox.style.background = "#ffdf00"; // Giallo se sta finendo
        if(secondiRimasti < 0) secondiRimasti = 0;
        ...
        // Se il tempo scade ED È IL TUO TURNO, forzi il salto
        if (secondiRimasti === 0 && p.id === mioId) {
            clearInterval(timerIntervallo);
            
            // Se scade in Fase Rinforzi, le piazza a caso per non fartele perdere!
            if (rFase === 0 && rTruppeDaPiazzare > 0) {
                ...
                syncRisikoNetwork("piazza_truppa_auto");
            }
            
            passaFaseManuale(true); // Parametro true per "Forzato"
        }
    }, 1000);
}
...
function controllaVittoriaGlobale() {
    ...
    let count = mieZone.length; let won = false;
...
function creaMazzo() { mazzoRisiko = []; Object.keys(R_NODES).forEach((k, index) => { mazzoRisiko.push({ tipo: TIPI_CARTA[index % 3], terr: R_NODES[k].nome }); }); mazzoRisiko.push({ tipo: "🃏", terr: "JOLLY" }); mazzoRisiko.push({ tipo: "🃏", terr: "JOLLY" }); mazzoRisiko.sort(() => Math.random() - 0.5); }

function pescaCartaRisiko(player) {
    if(mazzoRisiko.length > 0) {
        let carta = mazzoRisiko.pop(); player.cards.push(carta);
        if(!player.isBot) {
...
window.apriCarteRisiko = function() {
    ...
    let listDiv = document.getElementById("r-cards-list"); let modal = document.getElementById("risiko-cards-modal");
...
window.scambiaCarte = function() {
    let p = rPlayers.find(pl => pl.id === (isMultiplayerRisiko ? socket.id : "p0"));
    p.cards.splice(0, 3); rTruppeDaPiazzare += 8; 
    if(typeof window.suonaEffetto === 'function') try{ window.suonaEffetto('cassa'); }catch(e){}
    document.getElementById("risiko-carte-count").innerText = p.cards.length; document.getElementById("risiko-truppe-disp").innerText = rTruppeDaPiazzare;
    chiudiCarteRisiko();
...
function distribuisciTerritoriIniziali() {
    let keys = Object.keys(R_NODES); keys.sort(() => Math.random() - 0.5); let pIdx = 0;
    keys.forEach(k => { R_NODES[k].owner = rPlayers[pIdx].id; R_NODES[k].troops = 3; pIdx = (pIdx + 1) % rPlayers.length; });
    aggiornaMappaRisiko();
}

function ottieniIconaArmata(numero) { if (numero >= 10) return "🐘"; if (numero >= 5) return "🛺"; return "🛵"; }

function aggiornaMappaRisiko() {
    Object.keys(R_NODES).forEach(id => {
        let n = R_NODES[id]; let p = rPlayers.find(pl => pl.id === n.owner);
        let nodeDiv = document.getElementById("rn-" + id); let textDiv = document.getElementById("rnt-" + id);
        if(p && nodeDiv && textDiv) {
            nodeDiv.style.background = p.color; textDiv.innerHTML = `${n.troops} <span style="font-size:1rem;">${ottieniIconaArmata(n.troops)}</span>`;
        }
    ...
    if (rNodeSelected) {
        ...
        R_NODES[rNodeSelected].links.forEach(lId => {
            if (rFase === 1 && R_NODES[lId].owner !== R_NODES[rNodeSelected].owner) document.getElementById("rn-" + lId).classList.add("target");
            else if (rFase === 2 && R_NODES[lId].owner === R_NODES[rNodeSelected].owner) document.getElementById("rn-" + lId).classList.add("move-target");
        });
    }
...
function iniziaTurnoRisiko(idx) {
    rTurnoIndex = idx; let p = rPlayers[idx];
    ...
    if(territoriPosseduti === 0) { iniziaTurnoRisiko((idx + 1) % rPlayers.length); return; }
    
    rTruppeDaPiazzare = Math.max(3, Math.floor(territoriPosseduti / 3));
    impostaFaseRisiko(0); 
    
    if(p.isBot && !isMultiplayerRisiko) { setTimeout(() => eseguiTurnoBot(p), 1000); }
}

function passaFaseManuale(forzatoDalTimer = false) {
    let p = rPlayers[rTurnoIndex]; let mioId = isMultiplayerRisiko ? socket.id : "p0";
...
function impostaFaseRisiko(fase) {
    rFase = fase; let p = rPlayers[rTurnoIndex]; let isMyTurn = (p.id === (isMultiplayerRisiko ? socket.id : "p0"));
    
    let titolo = document.getElementById("risiko-fase-titolo"); let info = document.getElementById("risiko-info-turno");
    let btn1 = document.getElementById("btn-risiko-azione1"); let btn2 = document.getElementById("btn-risiko-azione2");
    
    btn2.onclick = () => passaFaseManuale(false);
    
    // Riparte il timer ad ogni cambio fase!
    avviaTimerRisiko();
...
window.clickNodoRisiko = function(id) {
    let p = rPlayers[rTurnoIndex]; let mioId = isMultiplayerRisiko ? socket.id : "p0";
    if(p.id !== mioId) return; let n = R_NODES[id];
    
    if (rFase === 0) {
        if (n.owner === p.id && rTruppeDaPiazzare > 0) {
            n.troops++; rTruppeDaPiazzare--;
            if(typeof window.suonaEffetto === 'function') try{ window.suonaEffetto('cassa'); }catch(e){}
            aggiornaMappaRisiko(); syncRisikoNetwork("piazza_truppa");
        ...
        if (n.owner === p.id) {
            ...
            // 🔥 CHIAMA LA FUNZIONE CHE PREPARA I DADI E LI SPARA IN BROADCAST 🔥
        ...
        if (n.owner === p.id) {
            if (!rNodeSelected) {
                ...
                if (rNodeSelected === id) {
                    ...
                    aggiornaMappaRisiko(); syncRisikoNetwork("sposta_truppa");
...
// 🔥 PREPARA I DADI E INVIA IL BROADCAST A TUTTI 🔥
function preparaBattagliaInDiretta(attId, defId) {
    ...
    // Genero i risultati VERI subito
    ...
    // Dico al server che sto attaccando, mando i nomi, i quartieri e i dadi finali!
    syncRisikoNetwork("inizio_battaglia", { 
        attId: attId, defId: defId, resAtt: resAtt, resDef: resDef 
    });
    
    // Eseguo l'animazione per me stesso (sono l'attaccante = true)
    eseguiAnimazioneBattaglia(attId, defId, resAtt, resDef, true);
}

// 🔥 L'ANIMAZIONE CHE VEDONO TUTTI IN CONTEMPORANEA 🔥
function eseguiAnimazioneBattaglia(attId, defId, resAtt, resDef, isAttaccante) {
    let nAtt = R_NODES[attId]; let nDef = R_NODES[defId];
    
    // Trovo i nomi dei giocatori per fare spettacolo
    let nomeAttaccante = rPlayers.find(pl => pl.id === nAtt.owner).name;
    let nomeDifensore = rPlayers.find(pl => pl.id === nDef.owner).name;
    ...
    // Aggiorno la grafica con Nomi e Quartieri!
    document.getElementById("r-battle-title").innerText = `${nomeAttaccante} ATTACCA ${nomeDifensore}!`;
    document.getElementById("r-battle-desc").innerText = `${nAtt.nome} ➔ ${nDef.nome}`;
    document.getElementById("r-battle-att-name").innerText = "⚔️ " + nomeAttaccante;
    document.getElementById("r-battle-def-name").innerText = "🛡️ " + nomeDifensore;
    ...
    let rollCount = 0;
    let numDadiAtt = resAtt.length; let numDadiDef = resDef.length;
    
    let anim = setInterval(() => {
        // Effetto finte rotazioni dadi
        ...
        if (rollCount >= 15) {
            ...
            // Applica i dadi reali decisi dall'Attaccante!
            ...
            // 🔥 SOLO L'ATTACCANTE AGGIORNA LA MAPPA E IL SERVER! GLI SPETTATORI GUARDANO E BASTA 🔥
            if (isAttaccante) {
                ...
                if (nDef.troops <= 0) {
                    ...
                    resText.innerHTML = `${nomeAttaccante} perde ${armatePerseAtt} 🛵<br>${nomeDifensore} perde ${armatePerseDef} 🛵`;
                    if(nAtt.troops === 1) rNodeSelected = null; 
                }
                
                aggiornaMappaRisiko();
                syncRisikoNetwork("fine_battaglia_risultato"); // Dice a tutti di aggiornare la mappa
                controllaVittoriaGlobale();
            } else {
                // Gli spettatori mostrano solo il testo corretto senza toccare le truppe localmente (ci penserà il sync)
    ...
    setTimeout(() => {
        ...
        if(attaccanti.length > 0) {
            ...
            if(bersagli.length > 0) {
                // I BOT ORA USANO LA NUOVA ANIMAZIONE VISIBILE!
